...
})({"ffri5":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "4137a9aa97432950";
"use strict";
...
},{}],"lfALp":[function(require,module,exports,__globalThis) {
...
const showMessage = async (text)=>{
    ...
    if (text === "Invalid Word") messageBox.classList.add("neon-red-box");
    else if (text === "\uD83C\uDF89 You Win!") messageBox.classList.add("neon-green-box");
    else messageBox.classList.add("neon-yellow-box");
    if (body.classList.contains("dark-theme")) messageBox.classList.add("dark-theme");
    await (0, _gsap.gsap).set(messageBox, {
        opacity: 0,
        y: -30,
        display: "block"
    });
    await (0, _gsap.gsap).to(messageBox, {
        opacity: 1,
        y: 0,
        duration: 0.6,
        ease: "power2.out"
...
const commitGuess = async ()=>{
    ...
    for(let i = 0; i < wordLength; i++)if (guessLetters[i] === correctLetters[i]) {
        status[i] = "correct";
    ...
    if (buffer === correctWord) {
        ...
        gameOver = true;
        const rowTiles = [];
        for(let i = 0; i < wordLength; i++)rowTiles.push(tilesArray[currentRow * columns + i]);
        // Wave bounce using timeline (ensures it lands back)
        const bounceTimeline = (0, _gsap.gsap).timeline();
...
document.addEventListener("keydown", async (event)=>{
    ...
    } else if (key === "ENTER" && buffer.length === wordLength) await commitGuess();
    else if (key === "BACKSPACE") applyBackSpace();
    processing = false;
});
// On-screen keyboard
document.addEventListener("click", async (event)=>{
...
},{"./gsap-core.js":"evdHe","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["ffri5","lfALp"], "lfALp", "parcelRequire5444", {})

//# sourceMappingURL=wordle.97432950.js.map
